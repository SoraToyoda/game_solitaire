...
void draw_game(GameState* game) {
    ...
    if (game->stock[game->stock_pos].value != 0) {
        draw_card(2, 70, &game->stock[game->stock_pos]);
    } else {
        mvprintw(2, 70, "+-------+");
        mvprintw(3, 70, "|       |");
        mvprintw(4, 70, "|  --   |");
    ...
    for (int f = 0; f < 4; ++f) {
        ...
        if (top >= 0) {
            draw_card(2, fx, &game->foundation[f][top]);
        } else {
            mvprintw(2, fx, "+-------+");
            mvprintw(3, fx, "|       |");
            mvprintw(4, fx, "|  --   |");
    ...
    for (int col = 0; col < 7; ++col) {
        ...
        for (int i = 0; i <= top; ++i) {
            ...
            draw_card(8 + i * 2, 2 + col * 10, &game->tableau[col][i]);
            if (selected) {
    ...
    mvprintw(28, 0, "←→: 移動 SPACE:選択/解除 s:山札をめくる d:場に出す f:場→組札 g:山札→組札 q:終了");
    mvprintw(29, 0, "複数選択: 場札でSPACE→↑↓で枚数選択→もう一度SPACEで確定→移動先でSPACEで移動");
    refresh();
}
